"""Demonstrate fitting a "real" (jaxoplanet) transit plus rotation-like SHO
signal with tinygp."""
...
LIMB_DARKENING = jnp.array([0.2, 0.1])
...
def transit_model(time, params):
    ...
    radius_ratio = jnp.sqrt(jnp.clip(jnp.exp(params["log_depth"]), 1e-9, None))
    orbit = TransitOrbit(
        period=period,
        duration=duration,
    ...
    flux_drop = limb_dark_light_curve(orbit, LIMB_DARKENING)(time)
    return flux_drop
...
if __name__ == "__main__":
    ...
    if true_transit_times.size > 0:
        ax_components.plot(
            true_transit_times,
            np.full_like(true_transit_times, np.max(flux_np - map_baseline) + 0.002),
            "v",
            color="C3",
            ms=5,
            label="true transits",
